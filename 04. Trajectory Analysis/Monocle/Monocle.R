...
dir.create('GO')
error_file <- file("GO/error_messages.txt", "a")                             # 这行代码的作用是打开（或创建）一个名为 "error_messages.txt" 的文件，以追加模式进行写入。
writeLines(c("Error log for GO enrichment analysis:",                        # 这意味着任何写入这个文件的数据都会被添加到文件的末尾，而不会覆盖原有内容
             "------------------------------------------------",
