...
#' clustering results in the metadata of the input Seurat object.
#'
#' @title Single-Cell Harmony Batch Processing with Multi-Resolution Clustering
#' @param seurat_obj Input Seurat object containing scRNA-seq data.
#' @param max_dim_pca Number of principal components used for Harmony.
#' @param max_dim_harmony Number of Harmony dimensions used for clustering/UMAP.
#' @param resolutions Numeric vector of clustering resolutions.
#' @param max_iter Maximum number of Harmony iterations.
#' @param top_n Number of top marker genes retained per cluster.
...
#' @param dpi Resolution of output plots.
#' @param out_dir Output directory.
#' @return Seurat object with all resolution-specific clustering results.
#' @author WWY
#' @export
...
sc_resolutionfinder <- function(seurat_obj = NULL,
  ...
  if (methods == "SCT") {
    ...
    seurat_obj <- SCTransform(seurat_obj, assay = "RNA", new.assay.name = "SCT", variable.features.n = 3000, vst.flavor = "v2", verbose = FALSE)
    DefaultAssay(seurat_obj) <- "SCT"
    seurat_obj <- RunPCA(seurat_obj, assay = "SCT", features = VariableFeatures(seurat_obj), verbose = FALSE)
  } else {
    message("Starting LogNormalize -> FindVariableFeatures -> ScaleData -> PCA...")
    DefaultAssay(seurat_obj) <- "RNA"
    seurat_obj <- NormalizeData(seurat_obj, normalization.method = "LogNormalize", scale.factor = median(seurat_obj$nCount_RNA), verbose = FALSE)
  ...
  message("Normalization method: ", methods)

  # Run Harmony correction based on current PCA
  ...
  for (res in resolutions) {
    ...
    # ---------------------- Pre-Harmony: Clustering & Visualization ----------------------
